/**
 * Annotate #hashtags with entities. Only executed once when the editor loads,
 * in order to decorate hashtags that could be present in the prepopulated text.
 * Hashtags written by users after the editor loads are handled by
...
const addPrepopulatedHashtag = (editorState, contentBlock, indices) => {
  ...
  contentState = contentState.createEntity('PREPOPULATED_HASHTAG', 'IMMUTABLE');
...
export default addPrepopulatedHashtag;
